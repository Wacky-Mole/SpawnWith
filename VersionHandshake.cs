...
namespace SpawnWith
{
    ...
    public static class RpcHandlers
    {
        ...
        public static void RPC_PieceManagerModTemplate_Version(ZRpc rpc, ZPackage pkg)
        {
            ...
            ModTemplatePlugin.PieceManagerModTemplateLogger.LogInfo("Version check, local: " +
                                                                   ModTemplatePlugin.ModVersion +
                                                                   ",  remote: " + version);
            if (version != ModTemplatePlugin.ModVersion)
            {
                PieceManagerModTemplatePlugin.ConnectionError =
                    $"{ModTemplatePlugin.ModName} Installed: {PieceManagerModTemplatePlugin.ModVersion}\n Needed: {version}";
                if (!ZNet.instance.IsServer()) return;
                // Different versions - force disconnect client from server
                ModTemplatePlugin.PieceManagerModTemplateLogger.LogWarning(
            ...
            else
            {
                if (!ZNet.instance.IsServer())
                {
                    ...
                    ModTemplatePlugin.PieceManagerModTemplateLogger.LogInfo("Received same version from server!");
                }
                else
                {
                    // Add client to validated list
                    ModTemplatePlugin.PieceManagerModTemplateLogger.LogInfo(
